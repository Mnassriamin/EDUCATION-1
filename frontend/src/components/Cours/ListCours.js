...
function ListCours() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Filter courses for teachers to show only their own
        const filteredCourses =
          userRole === "enseignant"
            ? data.filter((course) => course.enseignant._id === userId)
            : data;
    ...
    fetchCourses();
  }, [userRole, userId]); // ✅ Dependencies included
  ...
  const handleEnroll = async (courseId) => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/cours/${courseId}/enroll`,
        {
          method: "PUT",
      ...
      if (!res.ok) {
        throw new Error(
          responseData.message || "Erreur lors de l'inscription au cours"
        );
      }

      setCourses((prevCourses) =>
        prevCourses.map((course) =>
          course._id === responseData._id ? responseData : course
        )
      );
  ...
  const handleDelete = async (courseId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/cours/${courseId}`,
        { method: "DELETE" }
      );
            ...
            <li key={course._id} className="cours-item">
              ...
              {userRole === "enseignant" && (
                <div className="student-list">
                  <p><strong>Étudiants inscrits :</strong></p>
                  <ul>
                    {course.etudiants.length > 0 ? (
                      course.etudiants.map((student) => (
              ...
              {/* Students can enroll if not already enrolled */}
              {userRole !== "enseignant" &&
                !course.etudiants.some((e) => e._id === userId) && (
                  <button
